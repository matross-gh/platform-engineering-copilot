...
namespace Platform.Engineering.Copilot.Compliance.Agent.Services.Infrastructure;

/// <summary>
/// Infrastructure remediation service for automatically fixing compliance findings
/// through Azure Resource Manager API calls and configuration changes
/// </summary>
public class InfrastructureRemediationService : IInfrastructureRemediationService
{
    private readonly IAzureResourceService _resourceService;
    private readonly HttpClient _httpClient;
    private readonly ILogger<InfrastructureRemediationService> _logger;
    ...
    public InfrastructureRemediationService(
        IAzureResourceService resourceService,
        HttpClient httpClient,
        ILogger<InfrastructureRemediationService> logger)
    {
        _resourceService = resourceService;
        _httpClient = httpClient;
        _logger = logger;
        
        // Initialize Azure ARM client for Azure Government
        ...
        _armClient = new ArmClient(new DefaultAzureCredential(credentialOptions), defaultSubscriptionId: null, armClientOptions);

        // Initialize remediation templates
        _remediationTemplates = InitializeRemediationTemplates();

        _logger.LogInformation("✅ Infrastructure Remediation Service initialized with {TemplateCount} remediation templates", 
            _remediationTemplates.Count);
    }
