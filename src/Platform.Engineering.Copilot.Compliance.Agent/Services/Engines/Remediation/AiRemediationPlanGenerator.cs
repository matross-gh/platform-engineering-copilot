...
public class AiRemediationPlanGenerator : IAiRemediationPlanGenerator
{
    ...
    public bool IsAvailable => _chatCompletion != null;
...
{{
  ...
  ""estimatedMinutes"": 30
            ...
            chatHistory.AddSystemMessage("You are an Azure compliance remediation expert. Generate precise, production-ready remediation plans. Always return valid JSON.");
...
Include comments explaining each step. Make the script idempotent.";
        ...
        var script = new RemediationScript
        {
            ...
            Script = ExtractCodeFromResponse(response.Content ?? string.Empty),
...
Example: [{{""FindingId"": ""F-001"", ""ControlId"": ""AC-2"", ""Priority"": 1, ""Reasoning"": ""Critical security gap""}}]";
        ...
        var chatHistory = new ChatHistory();
        chatHistory.AddSystemMessage("You are a compliance risk analyst. Prioritize findings for maximum security impact with minimal disruption. Always return valid JSON.");
        chatHistory.AddUserMessage(prompt);

        var response = await _chatCompletion.GetChatMessageContentAsync(chatHistory, cancellationToken: cancellationToken);
        var prioritized = JsonSerializer.Deserialize<List<PrioritizedFinding>>(
            ExtractJsonFromResponse(response.Content ?? "[]")) ?? new List<PrioritizedFinding>();
