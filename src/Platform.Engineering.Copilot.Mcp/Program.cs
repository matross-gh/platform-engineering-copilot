...
class Program
{
    ...
    static async Task RunStdioModeAsync()
    {
        ...
        // Register database context (Cosmos DB)
        var configuration = builder.Configuration;
        var cosmosEndpoint = configuration["CosmosDb:Endpoint"]
            ?? configuration.GetConnectionString("DefaultConnection")
            ?? configuration.GetConnectionString("SqlServerConnection")
            ?? throw new InvalidOperationException("Cosmos DB endpoint not found. Set 'CosmosDb:Endpoint' or a connection string.");
        var cosmosKey = configuration["CosmosDb:Key"] ?? string.Empty;
        var cosmosDatabaseName = configuration["CosmosDb:DatabaseName"] ?? "PlatformEngineeringCopilot";
