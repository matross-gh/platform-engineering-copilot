...
namespace Platform.Engineering.Copilot.Agents.Discovery.Services.Azure;
...
public class AzureResourceDiscoveryService : IAzureResourceDiscoveryService
{
        ...
        IEnumerable<IResourceTypeHandler> resourceTypeHandlers)
    {
        ...
        _chatClient = chatClient; // Optional - graceful degradation if no AI configured
    ...
    {
        // If no AI client configured, return empty filters
        if (_chatClient == null)
        {
            _logger.LogDebug("No AI client configured, returning empty filters");
...
Only include fields that are mentioned in the query.";
    ...
    {
        // If no AI client configured, try basic extraction
        if (_chatClient == null)
        {
            _logger.LogDebug("No AI client configured, attempting basic resource extraction");
...
Example output: {{""resourceName"": ""mydata""}}";
    ...
    private async Task<(string? ResourceGroup, string? Subscription)> ParseInventoryScopeAsync(
        ...
        CancellationToken cancellationToken)
    {
        // If no AI client configured, return empty scope
        if (_chatClient == null)
        {
            _logger.LogDebug("No AI client configured, returning empty scope");
            return (null, null);
        }
...
Example: {{""resourceGroup"": ""rg-prod""}}";
    ...
    private async Task<Dictionary<string, string>> ParseTagFiltersAsync(string query, CancellationToken cancellationToken)
    {
        ...
        if (_chatClient == null)
        {
            _logger.LogDebug("No AI client configured, returning empty tag filters");
...
If only a tag key is mentioned without a value, use empty string as value.";
        ...
        var messages = new List<ChatMessage> { new(ChatRole.User, prompt) };
        var result = await _chatClient.GetResponseAsync(messages, cancellationToken: cancellationToken);
        // Note: In production, parse the JSON result properly
        _ = result.Text;
