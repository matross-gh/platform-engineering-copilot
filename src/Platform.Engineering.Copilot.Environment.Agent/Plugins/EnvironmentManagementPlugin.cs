...
public class EnvironmentManagementPlugin : BaseSupervisorPlugin
{
    ...
    public async Task<string> CreateEnvironmentAsync(
        [Description("Environment name (required)")] string? environmentName = null,
        [Description("Environment type: 'aks', 'webapp', 'function', 'containerapp' (default: aks)")] string? environmentType = null,
        [Description("Resource group name (required)")] string? resourceGroup = null,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(environmentName))
            {
                ...
                {
                    success = false,
                    error = "environmentName is required.",
                    hint = "Provide environmentName, resourceGroup, and location."
            ...
            if (string.IsNullOrWhiteSpace(resourceGroup))
            {
                ...
                {
                    success = false,
                    error = "resourceGroup is required for environment creation."
                }, new JsonSerializerOptions { WriteIndented = true });
            }

            _logger.LogInformation("Creating environment: {Name} in {ResourceGroup}", 
                environmentName, resourceGroup);

            var creationRequest = BuildDirectEnvironmentRequest(
                environmentName, 
                environmentType, 
                resourceGroup, 
                location,
            ...
            // 🔥 RETRIEVE GENERATED BICEP FILES FROM SHAREDMEMORY
                ...
                try
                {
                    ...
                    if (availableFiles != null && availableFiles.Count > 0)
                    {
                        _logger.LogInformation(
                            "📂 Retrieved {Count} generated files from SharedMemory for conversation {ConversationId}",
                        ...
                        if (!string.IsNullOrEmpty(mainTemplate))
                        {
                            ...
                            if (!string.IsNullOrEmpty(templateContent))
                            {
                                ...
                                _logger.LogInformation(
                                    "✅ Main Bicep template retrieved: {FileName} ({Size} bytes)",
                                ...
                                if (additionalFiles.Any())
                                {
                                    ...
                                    foreach (var fileName in additionalFiles)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(fileContent))
                                        {
                                            ...
                                            _logger.LogInformation(
                                                "📄 Additional module retrieved: {FileName} ({Size} bytes)",
                                                fileName, fileContent.Length);
                                        }
                                    }

                                    _logger.LogInformation(
                                        "✅ Retrieved {Count} additional Bicep modules from SharedMemory",
                            ...
                            {
                                _logger.LogWarning(
                                    "⚠️ Main template {FileName} found but content is empty",
                                    mainTemplate);
                            }
                        }
                        else
